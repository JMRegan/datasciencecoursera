ADX_Duplicates_Col <-function(file1, file2, column) {
        ...
        ShareX <- intersect(readAD$paste(column),readX$paste(column))
        ...
        final <-readX %>% filter(paste(column) %in% ShareXdf$paste(column))
